...
north_i = course_info["north_start"]
east_i = course_info["east_start"]
north_f = course_info["north_end"]
east_f = course_info["east_end"]
...
dyn_init_guess = asb.DynamicsPointMass3DSpeedGammaTrack(
...
for resolution in resolutions:
    ...
    dyn = asb.DynamicsPointMass3DSpeedGammaTrack(
        mass_props=asb.MassProperties(mass=1151.8 * u.lbm),
        x_e=opti.variable(init_guess=dyns[-1].x_e),
        y_e=opti.variable(init_guess=dyns[-1].y_e),
        ...
        alpha=opti.variable(
            init_guess=dyns[-1].alpha,
            lower_bound=-15,
            upper_bound=15
        ),
        beta=np.zeros(N),
        bank=opti.variable(
            init_guess=dyns[-1].bank,
            lower_bound=np.radians(-180),
            upper_bound=np.radians(180),
        ),
    )
    ...
    thrust = 0.8 * dyn.mass_props.mass * 9.81
    ...
    opti.subject_to([
        altitude_agl / 1e3 > 50 * u.foot / 1e3,
        # altitude_agl / 1e3 < 2500 * u.foot / 1e3
    ])
