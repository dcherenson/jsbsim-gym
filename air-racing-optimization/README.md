...
## DEM-Backed Canyon Task

In this repo, the optimization scripts now default to the Black Canyon DEM clip
that already exists under `data/dem/black-canyon-gunnison_USGS10m.tif`.

The active terrain/course definition lives in `task_config.py`. You can also
override the DEM without editing code by setting environment variables before
running a script:

```bash
export AIR_RACING_DEM_PATH="data/dem/your-canyon.tif"
export AIR_RACING_DEM_BBOX="south,north,west,east"
export AIR_RACING_DEM_START_PIXEL="x,y"
export AIR_RACING_DEM_SPAN_FT="9000"
```

Optional overrides:

```bash
export AIR_RACING_DEM_FINISH_PIXEL="x,y"
export AIR_RACING_CROP_PADDING_M="3000"
export AIR_RACING_FLY_DIRECTION="south_to_north"
```

If you have the original Riffe Lake raw tiles and want the legacy behavior
instead, set:

```bash
export AIR_RACING_TASK="legacy_riffe"
```
